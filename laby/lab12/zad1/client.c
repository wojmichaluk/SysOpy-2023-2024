...
int main(int argc, char* argv[]) {
    ...
    if (connect(socket_no, (const struct sockaddr*)&server_addr, sizeof(server_addr)) == -1) {
        perror("Blad ustawienia gniazda po stronie klienta!\n");
        cleanup();
    }
...
void init_connection(int socket_no, char *id) {
    ...
    if (send(socket_no, (const void*)&buffer, sizeof(buffer), 0) == -1) {
        perror("Blad w wysylaniu wiadomosci inicjujacej po stronie klienta!");
        return;
    }
